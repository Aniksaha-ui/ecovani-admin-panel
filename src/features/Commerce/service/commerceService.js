...
const list = async (path, { page = 1, search = "" } = {}) => {
  ...
  const meta =
    check(await apiRequest(`${path}?${query}`), "Unable to load records.") ||
    {};
...
export const getOrder = async (id) =>
  check(await apiRequest(`/admin/orders/${id}`), "Unable to load order.");
export const updateOrderTracking = (id, body) =>
  send(`/admin/orders/${id}/tracking`, "POST", body);
export const getTransactions = (params) => list("/admin/transactions", params);
export const getTransaction = async (id) =>
  check(
    await apiRequest(`/admin/transactions/${id}`),
    "Unable to load transaction.",
  );
export const getCoupons = (params) => list("/admin/coupons", params);
export const saveCoupon = (values, id) =>
  send(
    id ? `/admin/coupons/${id}` : "/admin/coupons",
    id ? "PATCH" : "POST",
    values,
  );
