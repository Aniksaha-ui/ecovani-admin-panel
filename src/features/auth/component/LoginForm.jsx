...
export default function LoginForm() {
  ...
  return (
    <form
      ...
      className="w-full max-w-[520px] rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] p-5 shadow-2xl shadow-black/30 sm:p-8"
    >
      <div className="mb-8">
      ...
      <label className="mb-5 block">
        ...
        <input
          ...
          className={`h-11 w-full rounded-md border bg-[var(--color-surface-raised)] px-3 text-sm text-white outline-none transition placeholder:text-[var(--color-text-faint)] focus:border-blue-500 ${
            errors.email
              ? "border-red-700"
        ...
        />
        {errors.email ? (
          <span className="mt-2 block text-xs font-medium text-red-300">
            {errors.email}
          </span>
        ) : null}
      </label>
      ...
      <label className="mb-6 flex items-center gap-2 text-sm font-medium text-[var(--color-text-muted)]">
        <input
          name="rememberMe"
          type="checkbox"
          checked={formValues.rememberMe}
          onChange={handleChange}
          className="h-4 w-4 rounded border-[var(--color-border-strong)] bg-[var(--color-surface-raised)]"
        />
