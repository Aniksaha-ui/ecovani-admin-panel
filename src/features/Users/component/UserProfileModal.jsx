...
function MetricCard({ label, toneClassName = "text-white", value }) {
  return (
    <article className="rounded-lg border border-[var(--color-border-strong)] bg-[var(--color-surface)] p-4">
...
function EmptyState({ message }) {
  return (
    <div className="flex min-h-[180px] items-center justify-center rounded-lg border border-dashed border-[var(--color-border-strong)] bg-[var(--color-surface)] px-5 py-8 text-center text-sm font-medium text-[var(--color-text-muted)]">
      {message}
    </div>
  );
}
...
export function UserProfileModal({
  ...
  return (
    <div className="crud-modal" role="dialog" aria-modal="true">
      <button
        type="button"
        className="crud-modal__backdrop"
      ...
      >
        <header className="crud-modal__header">
          <div className="min-w-0">
            ...
            <p className="mt-2 text-sm text-[var(--color-text-muted)]">
              Review bookings, support complaints, refund history, visa
              activity, and customer summary from one place.
            </p>
          </div>
        ...
        <div className="max-h-[72vh] overflow-y-auto p-4 sm:p-5">
          {isLoading ? (
            <div className="flex min-h-[280px] items-center justify-center rounded-lg border border-[var(--color-border-strong)] bg-[var(--color-surface)] text-sm font-semibold text-[var(--color-text-muted)]">
            ...
            <>
              <section className="mb-5 grid gap-3 md:grid-cols-4">
                <MetricCard
                  label="Accounts Spend"
                  toneClassName="text-white"
                  value={activeProfile.summary.totalSpentLabel}
                />
                <MetricCard
                  label="Total Bookings"
                  toneClassName="text-cyan-200"
                  value={activeProfile.summary.totalBookings}
                />
                <MetricCard
                  label="Tickets"
                  toneClassName="text-amber-200"
                  value={activeProfile.summary.totalTickets}
                />
                <MetricCard
                  label="Visa Applications"
                  toneClassName="text-emerald-200"
                  value={activeProfile.summary.visaApplications}
                />
              </section>

              <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(0,1.15fr)]">
                <div className="space-y-5">
                  <div className="rounded-lg border border-[var(--color-border-strong)] bg-[var(--color-surface-raised)]">
                    <header className="flex min-h-[50px] items-center gap-2 border-b border-[var(--color-border)] px-4 sm:px-5">
                    ...
                    <div className="grid gap-3 p-4 md:grid-cols-2">
                      <MetricCard
                        label="Name"
                        value={activeProfile.user.name}
                      />
                      <MetricCard
                        label="Email"
                        value={activeProfile.user.email}
                      />
                      <MetricCard
                        label="Role"
                        value={activeProfile.user.roleLabel}
                      />
                      <MetricCard
                        label="Joined"
                        value={activeProfile.user.joinedAtLabel}
                      />
                      <MetricCard
                        label="Paid / Pending"
                        value={`${activeProfile.summary.paidBookings} / ${activeProfile.summary.pendingBookings}`}
                      />
                      <MetricCard
                        label="Cancelled / Refund Pending"
                        value={`${activeProfile.summary.cancelledBookings} / ${activeProfile.summary.refundPending}`}
                      />
                      <MetricCard
                        label="Last Booking"
                        value={activeProfile.summary.lastBookingAtLabel}
                      />
                      <MetricCard
                        label="Last Ticket"
                        value={activeProfile.summary.lastTicketAtLabel}
                      />
                    </div>
                  </div>
                  ...
                  <div className="rounded-lg border border-[var(--color-border-strong)] bg-[var(--color-surface-raised)]">
                    ...
                    <div className="grid gap-3 p-4 md:grid-cols-3">
                      <MetricCard
                        ...
                        value={`${activeProfile.summary.paidBookings} paid`}
                      />
                      <MetricCard
                        label="Support Health"
                        value={`${activeProfile.summary.openTickets} open`}
                      />
                      <MetricCard
                        label="Refund Queue"
                        value={`${activeProfile.summary.totalRefunds} requests`}
                      />
                    </div>
                  </div>
                ...
                <div className="space-y-5">
                        ...
                        activeProfile.bookings.map((booking) => (
                          ...
                          >
                            ...
                            <div className="mt-4 grid gap-3 md:grid-cols-2">
                              ...
                              <MetricCard
                                label="Extra Status"
                                value={booking.extraStatus}
                              />
                              {booking.visaApplicationNo ? (
                                <MetricCard
                                  label="Visa Link"
                                  value={`${booking.visaApplicationNo} • ${booking.visaCountry} • ${booking.visaType}`}
                                />
                              ) : booking.refundStatus !== "N/A" ? (
                                <MetricCard
                                  label="Refund"
                                  value={`${booking.refundAmountLabel} • ${booking.refundStatus}`}
                                />
                              ) : (
                                <MetricCard
                                  label="Notes"
                                  value="No additional linked records."
                                />
                              )}
                            </div>
                          </article>
                  ...
                  <div className="rounded-lg border border-[var(--color-border-strong)] bg-[var(--color-surface-raised)]">
                    <header className="flex min-h-[50px] items-center gap-2 border-b border-[var(--color-border)] px-4 sm:px-5">
                      ...
                      <h3 className="text-sm font-bold text-white">
                        Refunds & Visa Activity
                      </h3>
                    </header>

                    <div className="grid gap-5 p-4 xl:grid-cols-2">
                      <div>
                        <p className="mb-3 text-[11px] font-bold uppercase tracking-[0.08em] text-[var(--color-text-muted)]">
                          Refunds
                        </p>
                      ...
                      <div>
                        ...
                        {activeProfile.visaApplications.length ? (
                          <div className="space-y-3">
                            {activeProfile.visaApplications.map(
                              (application) => (
                                <article
                                  key={application.id}
                                  className="rounded-lg border border-[var(--color-border-strong)] bg-[var(--color-surface)] p-4"
                                >
                                  <div className="flex items-start justify-between gap-3">
                                    ...
                                    <StatusBadge
                                      ...
                                      toneClassName={
                                        application.statusToneClassName
                                      }
                                    />
                                  </div>
                  ...
                  <div className="rounded-lg border border-[var(--color-border-strong)] bg-[var(--color-surface)] p-4 text-sm text-[var(--color-text-muted)]">
                    This profile is loaded from the admin customer-profile
                    endpoint and stays connected to the live user management
                    list.
                  </div>
                </div>
              </section>
