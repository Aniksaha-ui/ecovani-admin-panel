...
const formatDateTimeLabel = (value, fallback = "-") => {
  ...
  return parsedValue.isValid()
    ? parsedValue.format("DD MMM YYYY, hh:mm A")
    : fallback;
};
...
const buildInitials = (name, email) => {
  const source =
    normalizeText(name, "").trim() || normalizeText(email, "").trim();
...
export const normalizeUser = (item = {}, index = 0, pagination = {}) => {
  ...
  return {
    ...
    serial,
    verifiedLabel: isVerified
      ? formatDateLabel(emailVerifiedAt, "Verified")
      : USERS_EMPTY_STATE.notVerified,
  };
};
...
export const buildUserMetrics = (rows = []) => {
  ...
  const roleCount = new Set(rows.map((row) => row.roleKey).filter(Boolean))
    .size;
...
export const getUserProfileStatusToneClassName = (status) => {
  ...
  if (
    ["cancelled", "cancle_booking", "rejected", "declined"].includes(statusKey)
  ) {
...
const normalizeBookingDisplayTitle = (booking) =>
  normalizeText(
    booking?.display_title ??
      ...
      booking?.hotel_name ??
      (booking?.application_no
        ? `Visa Application ${booking.application_no}`
        : ""),
    `Booking #${booking?.id ?? "-"}`,
  );
...
const normalizeUserProfileBooking = (booking = {}, index = 0) => ({
  ...
  extraStatusToneClassName: getUserProfileStatusToneClassName(
    normalizeBookingExtraStatus(booking),
  ),
  id: booking.id ?? `booking-${index}`,
  paymentLabel: normalizeBookingPaymentLabel(booking),
  refundAmountLabel: booking.refund_id
    ? formatUserProfileCurrency(booking.refund_amount)
    : "N/A",
  ...
  travelDateLabel: normalizeText(
    booking.departure_time ?? booking.check_in_date,
    "N/A",
  ),
...
const normalizeUserProfileVisa = (application = {}, index = 0) => ({
  ...
  countryAndTypeLabel: normalizeText(
    [application.country_name_snapshot, application.visa_type_snapshot]
      .filter(Boolean)
      .join(" • "),
    "N/A",
  ),
  ...
  packageLabel: normalizeText(
    application.visa_package_name ?? application.full_name,
    "N/A",
  ),
...
export const normalizeUserProfileDetails = (payload = {}) => {
  ...
  return {
    bookings: Array.isArray(payload?.bookings)
      ? payload.bookings.map((booking, index) =>
          normalizeUserProfileBooking(booking, index),
        )
      : [],
    refunds: Array.isArray(payload?.refunds)
      ? payload.refunds.map((refund, index) =>
          normalizeUserProfileRefund(refund, index),
        )
      : [],
    summary: {
      cancelledBookings: toNumber(summary.cancelled_bookings),
      lastBookingAtLabel: formatUserProfileDateTime(
        summary.last_booking_at,
        "N/A",
      ),
      lastTicketAtLabel: formatUserProfileDateTime(
        summary.last_ticket_at,
        "N/A",
      ),
    ...
    tickets: Array.isArray(payload?.tickets)
      ? payload.tickets.map((ticket, index) =>
          normalizeUserProfileTicket(ticket, index),
        )
      : [],
    user: {
    ...
    visaApplications: Array.isArray(payload?.visa_applications)
      ? payload.visa_applications.map((application, index) =>
          normalizeUserProfileVisa(application, index),
        )
      : [],
  };
};
  ...
  return {
    customers: customers.map((customer, index) => {
      ...
      return {
        ...
        avgBookingValueLabel: formatUserProfileCurrency(
          metrics.avg_booking_value,
        ),
        avgProfitMargin: toNumber(metrics.avg_profit_margin),
        avgProfitMarginLabel: formatUserProfilePercentage(
          metrics.avg_profit_margin,
        ),
        bookingGrowthDirection: normalizeText(
          metrics.booking_growth_direction,
          "flat",
        ),
        bookingGrowthPercentage: toNumber(metrics.booking_growth_percentage),
        bookingGrowthPercentageLabel: formatUserProfilePercentage(
          metrics.booking_growth_percentage,
        ),
        ...
        growthReferenceMonth: normalizeText(
          metrics.growth_reference_month,
          "Latest month",
        ),
        hotelBookings: toNumber(metrics.hotel_bookings),
        id: customer?.id ?? `comparison-${index}`,
        knownMarginCostLabel: formatUserProfileCurrency(
          metrics.known_margin_cost,
        ),
        knownMarginRevenueLabel: formatUserProfileCurrency(
          metrics.known_margin_revenue,
        ),
        ...
        revenueGrowthDirection: normalizeText(
          metrics.amount_growth_direction,
          "flat",
        ),
        revenueGrowthPercentage: toNumber(metrics.amount_growth_percentage),
        revenueGrowthPercentageLabel: formatUserProfilePercentage(
          metrics.amount_growth_percentage,
        ),
