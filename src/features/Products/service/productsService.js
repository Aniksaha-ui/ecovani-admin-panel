...
const paginationFor = (meta, count) => ({
...
export const getProducts = async ({ page = 1, search = "" } = {}) => {
  ...
  const payload = assertSucceeded(
    await apiRequest(`${API_URLS.admin.products}?${query}`),
    "Unable to load products.",
  );
  const meta = payload?.data ?? {};
  const rows = Array.isArray(meta.data) ? meta.data : [];
  const pagination = paginationFor(meta, rows.length);
  return {
...
export const getProductOptions = async () =>
  assertSucceeded(
    await apiRequest(API_URLS.admin.productOptions),
    "Unable to load product options.",
  ).data || {};
export const getProduct = async (id) =>
  assertSucceeded(
    await apiRequest(`${API_URLS.admin.products}/${id}`),
    "Unable to load product.",
  ).data;
export const getProductReport = async (id) =>
  assertSucceeded(
    await apiRequest(API_URLS.admin.productReport(id)),
...
export const saveProduct = async (values, id) => {
  ...
  Object.entries(values).forEach(([key, value]) => {
    if (key === "image" && value instanceof File) body.append("image", value);
      ...
      (value || []).forEach((sectionId) =>
        body.append("section_ids[]", String(sectionId)),
      );
  ...
  return assertSucceeded(
    await apiRequest(
      id ? `${API_URLS.admin.products}/${id}` : API_URLS.admin.products,
      { method: "POST", body },
    ),
...
export const removeProduct = async (id) => {
  assertSucceeded(
    await apiRequest(`${API_URLS.admin.products}/${id}`, { method: "DELETE" }),
