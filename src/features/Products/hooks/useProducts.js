...
import { emptyPagination, getProduct, getProductOptions, getProductReport, getProducts, removeProduct, saveProduct } from '../service/productsService'
export default function useProducts() {
  ...
  const report = async (id) => { try { return await getProductReport(id) } catch (err) { toast.error(err.message || 'Unable to load product report.'); return null } }
  ...
  return { items, pagination, page, setPage, search, setSearch, options, isLoading, isSaving, error, refresh: load, edit, report, save, destroy }
}
