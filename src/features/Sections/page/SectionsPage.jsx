...
import {
  ...
  listSections,
  saveSection,
} from "../service/sectionsService";

function SectionForm({ section, saving, onClose, onSave }) {
      ...
      <form className="admin-modal" onSubmit={submit}>
        <header className="admin-modal__header">
          <div>
            ...
            <p>
              Control homepage or catalog section ordering and availability.
            </p>
          </div>
...
export default function SectionsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const result = await listSections({ page, search });
  ...
  const save = async (values, id) => {
    setSaving(true);
    try {
      const result = await saveSection(values, id);
  ...
  const columns = [
    { id: "name", label: "Section", accessor: "name" },
    { id: "display_order", label: "Display order", accessor: "display_order" },
    {
      id: "is_active",
      label: "Status",
      render: (row) => (
    ...
    <main className="routes-page">
      <div className="routes-page__inner">
        ...
        <AdminDataTable
          ...
          resultLabel={`Showing ${items.length} of ${pagination.total || 0} sections`}
